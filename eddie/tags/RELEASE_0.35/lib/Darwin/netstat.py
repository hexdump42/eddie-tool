...
## Start Date	: 20040506
## 
## Description	: Library of classes that deal with a Darwin netstat list
##
## $Id$
##
########################################################################
...
class TCPtable(datacollect.DataCollect):
	...
	# get the tcp stats
	rawList = utils.safe_popen('/usr/sbin/netstat -anf inet -p tcp', 'r')
...
class UDPtable(datacollect.DataCollect):
	...
	# get the udp stats
	rawList = utils.safe_popen('/usr/sbin/netstat -anf inet -p udp', 'r')
...
class IntTable(datacollect.DataCollect):
	...
	rawList = utils.safe_popen('/usr/sbin/netstat -in', 'r')
...
class stats_ctrs(datacollect.DataCollect):
	...
	rawList = utils.safe_popen('/usr/sbin/netstat -s', 'r')
